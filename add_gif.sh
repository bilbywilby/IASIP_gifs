...
echo "------------------------------------------------------------"
]# Example logic for add_gif.sh using jq
# ... after download and optimization ...

# 1. Get metadata from user
read -p "Enter tags (comma separated, e.g., dennis,psycho): " TAGS_INPUT
read -p "Enter description: " DESC_INPUT

# 2. Convert to JSON list format
TAGS_JSON = $(echo "$TAGS_INPUT" | sed 's/, */", "/g' | sed 's/^/["/; s/$/"]/')

# 3. Create new JSON object
NEW_ENTRY = $(jq -n \
--arg name "$NAME" \
--arg desc "$DESC_INPUT" \
--argjson tags "$TAGS_JSON" \
'{filename: $name, description: $desc, tags: $tags}')

# 4. Append to manifest and save
jq ". + [$NEW_ENTRY]" gifs/index.json > gifs/index.json.tmp && mv gifs/index.json.tmp gifs/index.json

# 5. Commit and push the manifest update as well
git add "$DEST" gifs/index.json
git commit -m "Add $NAME and update manifest"
# ... rest of the script